...
const Employee = () => {
  ...
  useEffect(() => {
    axios.get(`http://localhost:5000/employee`).then((res) => setEmployeeInfo(res.data));
  }, []);
      ...
      <section className={`${styles.bg}`}>
        <div className={`${styles.bgLayer} py-6`}>
          <div className={`mx-auto container px-3 m-auto`}>
            <div className="grid grid-cols-1 sm:grid-cols-col-2 md:grid-cols-3 lg:grid-cols-4 gap-3 items-center justify-center">
              {employeeInfo.map((employee, i) => (
                <div key={i} className={`w-full sm:w-[270px] md:w-[270px] lg:w-[270px] xl:w-[270px] h-auto  m-auto px-2 py-10 bg-[#ffff] shadow-2xl border border-white rounded`}>
                  <div id="employee_img">
                    <img className="w-[60%] rounded-full m-auto border border-blue-700 p-[2px]" src={employee?.image} alt="" />
                  </div>
