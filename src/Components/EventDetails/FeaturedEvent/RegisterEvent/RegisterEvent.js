...
const RegisterEvent = ({ eventDetailsData }) => {
   ...
   /* useEffect(()=> {
      axios.get(`https://eventy-server.onrender.com/ticket-booking/${user?.uid}`)
      .then(data=> {
         selecting(data?.data);
         setTicket({
   ...
   const [data, loading, refetch] = useRefetch(`https://eventy-server.onrender.com/ticket-booking/${user?.uid + ':' + eventDetailsData?._id}`, {}, (data) => {
      selecting(data);
      setTicket({
   ...
   const bookings = () => {
      ...
      axios.put(`https://eventy-server.onrender.com/ticket-booking/${user?.uid + ':' + eventDetailsData?._id}`, {
         booking: booked,
      }).then(({ data }) => {
               ...
               <ul>
                  ...
                  <li class="px-2">
                     <div class="flex py-2 border-b items-center justify-between ">
                        ...
                        <div class="">
                           <div class="ticket-select">
                              <select ref={enterpriseSelection} onClick={enterprise} className='bg-[#00000017] text-slate-500 font-semibold text-base md:text-lg rounded p-2 mr-3 outline-yellow-500 cursor-pointer' id="ticket-select1">
                                 <option className='text-slate-500 font-semibold text-sm md:text-base' value="0">0 TICKET</option>
                     ...
                     <div class="flex py-2 border-b items-center justify-between">
                        ...
                        <div class="">
                           <div class="ticket-select">
                              <select ref={professionalSelection} name='professional' onClick={professional} className='bg-[#00000017] text-slate-500 font-semibold text-base md:text-lg rounded p-2 mr-3 outline-yellow-500 cursor-pointer' id="ticket-select2">
                                 <option className='text-slate-500 font-semibold text-sm md:text-base' value="0">0 TICKET</option>
                                 <option className='text-slate-500 font-semibold text-sm md:text-base' value="1">1 TICKET</option>
                     ...
                     <div class="flex py-2 border-b items-center justify-between">
                        ...
                        <div class="">
                           <div class="ticket-select">
                              <select ref={standardSelection} name='standard' onClick={standard} className='bg-[#00000017] text-slate-500 font-semibold text-sm md:text-lg rounded p-2 mr-3 outline-yellow-500 cursor-pointer' id="ticket-select3">
                                 <option className='text-slate-500 font-semibold text-sm md:text-base' value="0">0 TICKET</option>
                                 <option className='text-slate-500 font-semibold text-sm md:text-base' value="1">1 TICKET</option>
                                 <option className='text-slate-500 font-semibold text-sm md:text-base' value="2">2 TICKET</option>
                                 <option className='text-slate-500 font-semibold text-sm md:text-base' value="3">3 TICKET</option>
                                 <option className='text-slate-500 font-semibold text-sm md:text-base' value="4">4 TICKET</option>
                                 <option className='text-slate-500 font-semibold text-sm md:text-base' value="5">5 TICKET</option>
                              </select>
                           </div>
                        </div>
                     </div>
