...
const AllEmployee = () => {
   ...
   useEffect(() => {
      axios.get("https://eventy-server.onrender.com/employee").then((res) => {
         const { data } = res;
         setEmployee(data);
      });
   }, []);
   const handleDeleteUser = (id) => {
      ...
      // if (proceed) {
      //    const url = `https://eventy-server.onrender.com/delete-employee/${id}`;
      //    fetch(url, {
      //       method: "DELETE",
      //    })
      //       .then((res) => res.json())
               ...
               <tbody class="bg-gray-200">
                  {employee.map((employee) => (
                     ...
                     >
                        ...
                        <td class="px-16 py-2 font-medium text-slate-600 text-center">{employee.email}</td>
                        {/* <td> <Link to={''}></Link> </td> */}
                        <td class="px-16 py-2 ">
                           <span class="text-yellow-500 flex justify-center px-2">
                              ...
                              <button onClick={() => handleDeleteUser(employee._id)} className=''>
                                 <svg
                                    xmlns="http://www.w3.org/2000/svg"
