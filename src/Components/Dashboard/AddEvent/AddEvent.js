...
const AddEvent = () => {
    ...
    const handleEventAdd = async (e) => {
        ...
        const eventImage1 = await LocalImageConvertIntoDynamicImageLink(Array.from(e.target.eventImage1.files));
        const eventImage2 = await LocalImageConvertIntoDynamicImageLink(Array.from(e.target.eventImage2.files));
        const eventImage3 = await LocalImageConvertIntoDynamicImageLink(Array.from(e.target.eventImage3.files));
        ...
        for (let index = 0; index < dateDifference; index++) {
            day.push({
                ...
                date: stringDateConvertIntoDayMonthYear(e.target[`day${index + 1}Date`].value),
                ...
                image: await LocalImageConvertIntoDynamicImageLink(Array.from(e.target[`day${index + 1}Image`].files))
            })
        }
        ...
        fetch('https://fathomless-hamlet-59180.herokuapp.com/addEventRequest', {
            ...
            .then((data) => {
                if (data.acknowledged) {
                    toast.success(`Website Admin Receive Your Event Information. Very soon Admin Member Contact you`);
                    e.target.reset();
                    setLoading(false);
                } else {
                    ...
                    <div className='ml-8 sm:ml-5 mr-10 bg-white p-5 rounded-none sm:rounded-2xl'>
                        ...
                        <form onSubmit={handleEventAdd}>
                            ...
                            {[...Array(dateDifference).keys()].map((eventShedule, index) => <>
                                ...
                                <h3 className='text-2xl mt-10'>Day {eventShedule + 1} : Shedule Information</h3>

                                <div className='sm:flex md:flex-none'>
                                    <div className='grid grid-cols-1 gap-3 sm:w-1/2 mt-5'>
                                        <div className="form-control">
                                            <input type="text" name={`day${eventShedule + 1}Title`} placeholder="Event Title" className="input input-bordered inputDegine" />
                                        </div>
                                        <div className="form-control">
                                            <input type="text" name={`day${eventShedule + 1}Speaker`} placeholder="Event Host Speaker Name" className="input input-bordered inputDegine" />
                                        </div>
                                        <div className="form-control grid grid-cols-12 items-center ml-1">
                                            <p className='col-span-5 inputFontSize font-semibold'>Day {eventShedule + 1} - Event Date <span className='font-bold ml-3'>:</span></p>
                                            <input type="date" name={`day${eventShedule + 1}Date`} placeholder="Event Start Time" className="input input-bordered col-span-7 inputDegine" />
                                        </div>
                                        <div className="form-control grid grid-cols-12 items-center ml-1">
                                            <p className='col-span-5 inputFontSize font-semibold'>Day {eventShedule + 1} - Event Start Time <span className='font-bold ml-3'>:</span></p>
                                            <input type="time" name={`day${eventShedule + 1}starttime`} placeholder="Event Start Time" className="input input-bordered col-span-7 inputDegine" />
                                        </div>
                                        <div className="form-control grid grid-cols-12 items-center ml-1">
                                            <p className='col-span-5 inputFontSize font-semibold'>Day {eventShedule + 1} - Event End Time <span className='font-bold ml-3'>:</span></p>
                                            <input type="time" name={`day${eventShedule + 1}endtime`} placeholder="Event End Time" className="input input-bordered col-span-7 inputDegine" />
                                        </div>
                                        <div className="form-control">
                                            <input type="number" name={`day${eventShedule + 1}Seats`} placeholder="Total Event Seats (Only Digit Type)" className="input input-bordered inputDegine" />
                                        </div>
                                        <div className="form-control">
                                            <textarea name={`day${eventShedule + 1}Description`} className="textarea textarea-bordered h-32 inputDegine" placeholder={`About Tell Us Day ${eventShedule + 1} Event Shedule`} ></textarea>
                                        </div>
                                    </div>
                                    <div className='sm:pl-14 mt-3'>
                                        <div>
                                            <p className='text-xl font-semibold mb-3'> Day {eventShedule + 1} : Event Image 1 Minimum Include</p>
                                            <p className='label-text font-medium mb-1'>Event Image Day : {eventShedule + 1}</p>
                                            <div className='w-[294px] pr-7 bg-amber-400 relative rounded-md'>
                                                <FontAwesomeIcon className='absolute top-1/3 left-6 text-white' icon={faCloudArrowUp}></FontAwesomeIcon>
                                                <label class="block">
                                                    <input type="file" class="block w-full text-base text-gray-100 rounded-md file:mr-4 file:py-2 file:pr-0 file:pl-12 file:border-0 file:text-base file:font-semibold
                                                    file:bg-amber-400 file:text-white"
                                                        name={`day${eventShedule + 1}Image`} required />
                                                </label>
                                            </div>
                                        </div>
                                    </div>
